...
In a debt position, all three asset types (X, Y, and LPs) can participate, but each asset must serve exclusively either as collateral or as debt within a given loan, so each variable $X$, $Y$, $L_X$, & $L_Y$ can be negative or positive depending if it represents debt or collateral. If both $X + L_X < 0$ and $Y + L_Y < 0$, there is no collateral and the validation will fail and if both values are positive, there is no net debt and the validation of LTV passes. In the simpler case, LTV calculacion can be represented by the following table (let all variables are positive):

| Collateral \\ Debt  | **X**                  | **Y**                  | **LPs**               |
|---------------------|------------------------|------------------------|-----------------------|
| **X**               | Not possible           | $\frac{Y}{X}$          | $\frac{L_Y}{X - L_X}$ |
| **Y**               | $\frac{X}{Y}$          | Not possible           | $\frac{L_X}{Y - L_Y}$ |
| **LPs**             | $\frac{X - LP_X}{L_Y}$ | $\frac{Y - L_Y}{L_X}$  | Not possible          |
